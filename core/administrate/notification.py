...
async def set_photo(mess: types.Message, state: FSMContext, bot: Bot):
    ...
    await mess.answer("Выберите каким пользователям будет разослано сообщение:",
                      reply_markup=kbi.choice_people())
    await state.set_state(NotificationProcess.SetPeople)
    return


@subrouter.callback_query(F.data == "no", NotificationProcess.SetMessage)
async def set_photo_no(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(NotificationProcess.SetPeople)
    await call.message.edit_text("Выберите каким пользователям будет разослано сообщение:",
                                 reply_markup=kbi.choice_people())
    return


@subrouter.callback_query(F.data.split("_")[0] == "notif", NotificationProcess.SetPeople)
async def set_people(call: types.CallbackQuery, state: FSMContext):
    await state.update_data({"people": call.data.split("_")[-1]})
    data = await state.get_data()
    try:
        await call.message.delete()
        msg1 = await call.message.answer_photo(data["id_photo"])
        msg2 = await call.message.answer(data["text"])
        await state.update_data({"del1": msg1.message_id, "del2": msg2.message_id})
    except:
        msg1 = await call.message.answer(data["text"])
        await state.update_data({"del1": msg1.message_id})
    await call.message.answer("Если все верно, то я начинаю рассылку: ",
                              reply_markup=kbi.confirmation())
...
async def start_notif(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        pass
    if data["people"] == "courier":
        user_id = await get_id_courier()
    elif data["people"] == "customer":
        user_id = await get_id_customer()
    else:
        user_id = await get_id_all_user()
